...
if (!logPath || !fs.existsSync(logPath)) {
  console.error('Usage: analyze-rn087-snap-stress-log.mjs <log>');
  process.exit(2);
}
...
for (const line of lines) {
  ...
  const skip = line.match(
    /SOURCE_SNAP_DIRECT_SKIP reason=([A-Za-z0-9_-]+) targetY=(-?\d+) sourceVelocityY=(-?\d+) sourceY=(-?\d+)(?: maximumY=(-?\d+))?/,
  );
  if (skip) {
    stats.skips.push({
      reason: skip[1],
      ...
      maximumY: skip[5] == null ? null : Number(skip[5]),
  ...
  const start = line.match(
    /SOURCE_SNAP_DIRECT_START mode=post-only-target-lock targetY=(-?\d+) sourceVelocityY=(-?\d+) baselineY=(-?\d+) started=(true|false)(?: maximumY=(-?\d+))?/,
  );
  if (start) {
    ...
    const segment = {
      ...
      maximumY: start[5] == null ? null : Number(start[5]),
      requestTargetY: lastRequest?.targetY ?? null,
      requestVelocityY: lastRequest?.velocityY ?? null,
      frames: 0,
      requestedNetY: 0,
      childNetY: 0,
      touchDownWhileActive: false,
      touchSourceY: null,
      postTouchFrames: 0,
      ended: false,
      reason: null,
  ...
  const touchDown = line.match(
    /SOURCE_TOUCH action=DOWN directActive=(true|false) targetY=(-?\d+) maximumY=(-?\d+) sourceY=(-?\d+) scrollerY=(-?\d+) scrollerFinished=(true|false)/,
  );
  if (touchDown && touchDown[1] === 'true') {
    stats.touchDownWhileActive += 1;
    if (active != null && !active.touchDownWhileActive) {
      active.touchDownWhileActive = true;
      active.touchSourceY = Number(touchDown[4]);
    }
  }
  ...
  if (frame) {
    ...
    if (active == null) {
      ...
      if (active.touchDownWhileActive) active.postTouchFrames += 1;
      if (requestedY !== childConsumedY + remainingY) stats.brokenFrames += 1;
    }
  }

  const end = line.match(
    /SOURCE_SNAP_DIRECT_END reason=([A-Za-z0-9_-]+) targetY=(-?\d+) sourceY=(-?\d+) scrollerY=(-?\d+)(?: scrollerFinished=(true|false))?(?: maximumY=(-?\d+))?/,
  );
  if (end) {
    if (active == null) {
      stats.orphanEnds += 1;
      ...
      if (active.maximumY == null && end[6] != null) active.maximumY = Number(end[6]);
      active = null;
    }
  }
}
...
const interrupted = ended.filter(
  item => item.touchDownWhileActive && item.sourceY !== item.targetY,
);
...
const supersededWithoutTouch = ended.filter(
  item => item.reason === 'superseded' && !item.touchDownWhileActive,
);

let reversals = 0;
for (const item of interrupted) {
  const next = stats.segments[item.index] ?? null;
...
const topEdgeCompleted = completed.filter(item => item.maximumY != null && item.targetY === 0);
const bottomEdgeCompleted = completed.filter(
  item => item.maximumY != null && item.targetY === item.maximumY,
);
...
const interruptionPass =
  interrupted.length >= 2 &&
  interruptionMotionLeaks.length === 0 &&
  supersededWithoutTouch.length === 0;
const reversalPass = reversals > 0;
const edgePass = topEdgeCompleted.length > 0 && bottomEdgeCompleted.length > 0;

console.log(`RN 0.87 direct-snap stress report: ${path.resolve(logPath)}`);
console.log('');
console.log('Source');
...
console.log('');
console.log('Direct snap stress');
...
console.log(`  touch-down while active       ${stats.touchDownWhileActive}`);
console.log(`  post-touch old-snap frames    ${interruptionMotionLeaks.reduce((n, x) => n + x.postTouchFrames, 0)}`);
console.log(`  reversal pairs                ${reversals}`);
console.log(`  completed target matches      ${completed.length - completedTargetMismatches.length}/${completed.length}`);
console.log(`  completed top / bottom edge   ${topEdgeCompleted.length} / ${bottomEdgeCompleted.length}`);
console.log(`  velocity |min| / |max|        ${minVelocity} / ${maxVelocity}`);
console.log(`  broken/orphan/overlap         ${stats.brokenFrames} / ${stats.orphanFrames + stats.orphanEnds} / ${stats.overlappingStarts}`);
console.log(`  superseded without touch      ${supersededWithoutTouch.length}`);

if (interruptionMotionLeaks.length || completedTargetMismatches.length || supersededWithoutTouch.length) {
  console.log('  mismatch details');
  for (const item of [
    ...interruptionMotionLeaks,
    ...completedTargetMismatches,
    ...supersededWithoutTouch,
  ].slice(0, 10)) {
    console.log(
      `    #${item.index} reason=${item.reason} base=${item.baselineY} target=${item.targetY} ` +
        `sourceY=${item.sourceY} v=${item.sourceVelocityY} touch=${item.touchDownWhileActive} ` +
        `postTouchFrames=${item.postTouchFrames} max=${item.maximumY}`,
    );
  }
}
...
const gates = [
  ...
  ['top + bottom edge coverage', edgePass],
];
